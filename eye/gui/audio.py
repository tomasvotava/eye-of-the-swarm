...
class AudioManager:
    ...
    def __init__(
        ...
        self._ambient_channel = (
            ambient_channel if ambient_channel is not None else pygame.mixer.Channel(_AMBIENT_CHANNEL_ID)
        )
        self._battle_primary_channel = (
            battle_primary_channel
            if battle_primary_channel is not None
            else pygame.mixer.Channel(_BATTLE_PRIMARY_CHANNEL_ID)
        )
        self._battle_result_channel = (
            battle_result_channel
            if battle_result_channel is not None
            else pygame.mixer.Channel(_BATTLE_RESULT_CHANNEL_ID)
        )
